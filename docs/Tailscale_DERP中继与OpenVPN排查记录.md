...
- 日期：2026-04-12
- 环境：GL-MT2500 (gl-mt2500-3) 家庭软路由 ↔ iStoreOS (istoreos) 公司旁路由，通过 Tailscale 组网，自建 DERP 中继在阿里云 (bj-ali-hb2)

## 最终目标

1. **家庭 ↔ 公司互访**：通过 Tailscale 组网，22 服务器、gl-mt2500-3 能访问公司内网的 Bitbucket (192.168.199.94:7990)、istoreos (192.168.199.126) 等服务器
2. **低延迟 DERP 中继**：自建国内 DERP 中继（目标 < 20ms），摆脱海外官方节点（当前最近 nue 约 470ms）
3. **稳定可靠**：不依赖 OpenVPN tunnel-in-tunnel 等临时方案，Tailscale 直连或走国内 DERP 中继
...
|--------|-------------|------|------|------|
| gl-mt2500-3 | 100.83.20.58 | Linux (OpenWrt) | 家庭 | 主路由/软路由，OpenVPN 服务端 |
...
| 192.168.199.0/24 | 公司服务器段 | 与 100 段的互通方式不明；Bitbucket (.94:7990), 服务器 (.126) |
...
```
    ...
    └─────────────────────────────┘    │                                  │
     ...
     (DDNS: dff.x3322.net:5003)        │  │ 服务器:    .126      │        │
                                       │  └──────────────────────┘        │
              Tailscale WireGuard       └─────────────────────────────────┘
